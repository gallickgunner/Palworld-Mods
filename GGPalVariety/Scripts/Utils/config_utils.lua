...
local function ValidateConfig(config, default_config, schema)
	schema = schema or {}
	local write_back = false
	for key, def_value in pairs(default_config) do
		local user_value = config[key]
		local child_schema = schema[key] or {}

		if type(def_value) == "table" then
			if type(user_value) ~= "table" and not schema.__loose then
				config[key] = {}
				write_back = true
			end

			if not child_schema.__loose then
				if ValidateConfig(user_value, def_value, child_schema) then
					write_back = true
				end
			end
...
local function CreateUserConfigWithDefaults(config, config_path, default_config_path)
	...
	-- strip starting annotations and class variable name from default config
	default_config_contents = default_config_contents:gsub("^.-local%s+[%a_][%w_]*%s*=%s*{", "return {", 1)

	-- strip the return tablename at the very end
	default_config_contents = default_config_contents:gsub("%s*return%s+[%a_][%w_]*%s*$", "", 1)
...
local function UpdateUserConfigFile(config, config_schema, config_path, default_config_path)
	...
	local wrote = file:write(comments, "return ", SerializeValue(config, 0, config_schema), "\n")
	file:close()
...
function ConfigUtils.LoadConfigFile(config_path, default_config_path, default_config, config_schema)
	...
	local update_config = ValidateConfig(config, default_config, config_schema)

	if update_config and UpdateUserConfigFile(config, config_schema, config_path, default_config_path) then
