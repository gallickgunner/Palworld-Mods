...
local function IsBuiltInRandomizerEnabled(world_context)
	...
	if randomizer_manager:IsInitializedRandomizer() and randomizer_type ~= RANDOMIZER_TYPE_NONE then
		return true
	end
...
local function InitItemCache(world_context)
	if #saddle_items_cache ~= 0 or #weapon_saddles_cache ~= 0 then
		return true
	end
...
local function GrantRideItems(player_state)
	...
	if mod_config.rideability.grant_saddles then
	...
	if mod_config.rideability.grant_saddle_weapons then
...
local function GrantRideabilityItemsOnWorldLoad(player_state)
	if not IsValid(player_state) then
...
local function IsRidePalRestrictedBySize(pal)
	local mesh = pal.Mesh

	if not IsValid(mesh) then
		return nil
	end
	local pal_size = PalUtils.GetPalSize(pal)
	local size_key = PalUtils.PAL_SIZE[pal_size]
	...
	local target_min_ride_scale = mod_config.rideability.min_ride_scales[size_key]

	return curr_scale.X < target_min_ride_scale
end

local function IsRidePalRestrictedByTrust(pal)
	local indiv_param = CDO.pal_utility:GetIndividualCharacterParameterByActor(pal)
...
local function RegisterRideabilityRestrictionHooks()
	...
	RegisterHook(
		...
		function(Context, ReturnValue, Trainer)
			...
			local options = mod_config.rideability
			if options.restricted_by_size and IsRidePalRestrictedBySize(pal_actor) then
				--DebugLogActor("Pal size too small, locking pskill.", pal_actor)
				ReturnValue:set(true)
				return
			end

			if options.restricted_by_trust and IsRidePalRestrictedByTrust(pal_actor) then
				--DebugLogActor("Pal trust too low, locking pskill.", pal_actor)
				ReturnValue:set(true)
			end
		end
	)
...
function Ride.Init()
	...
	if options.restricted_by_size or options.restricted_by_trust then
		RegisterRideabilityRestrictionHooks()
	end

	if options.grant_saddle_weapons or options.grant_saddles then
		Server.RegisterOnNotifyWorldLoadedToServer(GrantRideabilityItemsOnWorldLoad)
	end
end
