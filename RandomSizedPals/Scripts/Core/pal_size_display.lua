...
local function CreateSizeRow(menu)
	...
	DebugLog("Native Pal size row created for %s", menu:GetFullName())
...
local function GetMinimumRideScale(pal_actor)
	...
	if not pal_size_key or not mod_config.min_ride_scales then
		return nil
	end

	return tonumber(mod_config.min_ride_scales[pal_size_key])
end
...
local function AddSizeRow(menu, handle)
	...
	if not PalUtils.IsPalRideableFromActor(pal_actor) or not mod_config.disable_tiny_ride_pals then
		row.popup_sub_info = nil
		return
	end

	row.popup_sub_info = string.format("\nRide %d\nSprint %d", run_speed, ride_sprint_speed)
	...
	row.popup_info = row.popup_info .. string.format("Min size for riding %d%%", ride_percent)
end
...
local function RegisterHoverHooks()
	NotifyOnNewObject(
		...
		function()
			...
			DebugLog("Hooking COMMON BUTTOn")
			return true
		end
	)
end
